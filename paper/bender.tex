...
\textbf{These allow us to quantify the strength of the pulse fractions one expects to observe from typical fasly spinning millisecond pulsars.}
}
...
In other words, irrespective of the unknown microphysics of the underlying matter, the NS parameters are bound by what is called an (approximative) universal relations.
\citet{BBP13} defined an empirical relations for these parameters in the Hartle-Thorne metric based on their computations of rotating NSs with various different equations of state.
...
Advantage of this alternative representation is its physical intuitiveness as the formalism relies heavily on identifying and using the constants of motion of the problem.
...
\footnote{\textbf{Real numerical infinity we use is around $\sim 10^5 \sch$ radii.}}
...
\textbf{Exact shapes of line profiles for different neutron star spinning $600~\mathrm{Hz}$.
Observer inclinations span a range from $i=5\deg$ (red), $10\deg$ (blue), $20\deg$ (green), $40\deg$ (purple), $60\deg$ (orange), to $90\deg$ (black).
Energy in the $x$-axis is scaled with the compactness $\sqrt{1-u} = \sqrt{1-2GM/R_{\mathrm{e}}}$ that would be expected from the gravitational redshift alone.
Likewise, the flux in the $y$-axis is always normalized with the peak flux to better show the evolution of the profile shape.
    }
}
\end{figure*}
...
\textbf{Line profile Full Width at Tenth Maximum (FWTM; top row) and Full Width at Half Maximum (FWHM; middle row) as a function of radius and observer inclination computed for different neutron star configurations of $M=1.1~\Msun$, $1.5~\Msun$, and $1.8~\Msun$ spinning $600~\mathrm{Hz}$.
Additionally, the bottom row shows the ratio of these two that can be used to quantify how narrow the upper part of the profile is.
Note the different inclination scale that is used to highlight the region $i < 10\deg$, where the profile evolves from a smooth to a spiky shape.
    }
}
\end{figure*}
...
From here it is easy to see that the profile appears smooth to almost all observer viewing angles.
...
\textbf{
We can also try and quantify the observed effect more thoroughly by introducing the Full Width at Half Maximum (FWHM) of the profile, in addition to Full Width at Tenth Maximum (FWTM). 
