...
On the other hand, our observer is stationary and moving along the timelike Killing vector, only.
...
\subsection{Emission}
Observed flux from a small area on image plane with photon energy $E$ is
\be
dF_E = I_E d\Omega_o,
\ee
...
\ee
The total (monochromatic) flux $F_E$ can then be obtained by integrating over the whole image
\begin{align}\begin{split}
F_E &= \int_0^{b_{\mathrm{max}}} \int_0^{2\pi} I_{E}(b,\chi) ~bdb \, d\chi \\
& = \int_0^{b_{\mathrm{max}}} \int_0^{2\pi} \frac{I'_{E'}(\alpha')}{(1+z)^3}  ~bdb \, d\chi\\ 
\end{split}\end{align}
