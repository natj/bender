...
The Full Width at Tenth Maximum and Full Width at Half Maximum of the smearing kernels are also reported for all of the possible viewing angles.
...
\refe{This connections is, however, done algebraically and there is no need to determine the full path of the ray as the problem reduces to calculating the so-called lensing integral only.}
In this method, the calculations are done in a special relativistic framework \refe{where quantities are defined in a co-rotating coordinate system that is then Lorentz boosted into the static non-rotating frame.}
We will now show how to define this \refe{change of frame properly, in order for the results to match the backwards-in-time method} (see also \citealt{CML07}, for an alternative derivation).
...
Typical photon propagation takes usually around a minute for a polar grid of $N_b \sim 100$ and $N_{\chi} \sim 100$ points.
...
For fast calculations there exists other more approximate ways to solve the problem, such as the oblate \sch method where the symmetries of the \sch spacetimes are extensively used and the ray tracing reduces to lensing angle integrals \citep[see e.g.][]{PB06, aGM14}.
