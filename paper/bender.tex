...
\author{J. N\"attil\"a\altaffilmark{1,2}\thanks{nattila.joonas@gmail.com},
  ...
  \red{J. Poutanen\altaffilmark{1,2}(?)}}
...
We give a detailed descriptions of the numerical algorithms used and provide an open source implementation of the framework available from \url{http://github.com/natj/bender}.
...
Accurately modeling the emission from and around compact objects is a complicated combination of radiative processes and relativity.
...
First of all, due to a finite pressure supporting the rotating star, the initially spherical star squeezes into an oblate spheroid when the rotation rates increase \citep{CST94, MLC07, aGM14}.
...
Here we seek to provide a similar, but open and publicly available code for solving similar type of problems.
...
However, in some cases we will simplify the equations into more intuitive form by using the \sch areal $r$ coordinate.
...
From here it is clear how the angles transform easily in the isotropic metric while the quantities related to the area are simplified in the usual \sch metric.
...
The relation between the emergent energy $E'$ to the observed energy $E$ is $E/E' = (1 + z)^{-1}$ (see \ref{eq:redshift}).
...
\be
d\Omega_o = \frac{\lgamma \delta}{1-u} \frac{d \cos\alpha}{d \cos\psi} ~\cos\alpha ~ \frac{dS''}{D^2}.
\ee
%as given by the co-rotating frame coordinates $dS'' = R^2 d\cos\theta d\phi'$.
It differs by a factor of $\lgamma$ from the S+D approximation that is valid for an infinitesimally small spots.
In other words, the surface element $dS'$ must be defined based on simultaneous measurements of local \textit{static} observers --- not using simultaneous measurements of comoving observers --- when dealing with finite sized spots on the surface.
...
Here the error in time might be interpreted as not taking into account the different photon arrival times to the detector.
...
Most notably, we employ an extensive comparison against AMP pulse profiles computed with a code that propagates photons forward in time as presented in \citealt{PB06} and in Sect.~\ref{sect:coords}.
...
We consider \sch spacetime for spherical and oblate stars (radius given by eq. \ref{eq:radf}), in addition to a full second-order spacetime with quadrupole moments and oblate surfaces.
...
Here some hot spot on top of the surface is emitting and the star is seen to rotate with a frequency of $\Omega$.
...
We defined a circular spot with angular radius $\rho$ either $1$ or $30$ degrees.
...
Angular distribution of radiation corresponds either to isotropic blackbody with constant intensity or an electron scattering dominated atmosphere, i.e., Hopf profile.
...
The surface here is defined using the radius function \eqref{eq:radf} but the star is still embed in a symmetric \sch spacetime.
...
Only large deviation is, again, seen when the spot is viewed from an extreme angle for $\theta_{\mathrm{s}} = 90^{\circ}$, just before the eclipse.
...
Cutting the skymap from one particular $i$ produces the light curve as seen by the observer.
Calculations here are done for an extreme case of a NS with $R_{\mathrm{e}} = 15~\mathrm{km}$, $M=1.6~\Msun$, and $\nu = 600~\mathrm{Hz}$.
Spot size of $\rho = 10^{\circ}$ is considered (now defined in the spherical co-rotating coordinate system using linear length scales), with a varying co-latitude ranging from near the pole at $\theta_{\mathrm{s}} = 10^{\circ}$ to a spot in the equator at $\theta_{\mathrm{s}} = 90^{\circ}$.
...
Full solid angle element is also derived to go beyond the S+D approximation of a infinitely small spots.
Most notably, this leads to a difference of $\lgamma$ (i.e., a simple Lorentz contraction in \sch metric) on the observed flux.
We also briefly discuss the actual intensity of the emerging radiation and present an iterative method to solve the Chandrasekhar-Ambartsumian integral (along an approximative polynomial expansion) that is related to the angle-dependent electron scattering atmosphere in Section \ref{sect:angular_distr}.
...
Lastly, and for the first time, we compute full skymaps of radiation emerging from rapidly rotating AMPs taking into account the oblate shape of the star and the quadrupole moments of the spacetime metric.
