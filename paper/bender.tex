...
\red{TODO: From where is it clear that the angles transform easily in the isotropic metric, and areas in the normal Sch metric? We must mention the equations or be otherwise specific, or there will be a referee comment about weasel words.}
...
From equation~\eqref{eq:redsfhit}, the relation between the emergent energy $E'$ to the observed energy $E$ is $E/E' = (1 + z)^{-1}$.
...
\red{TODO: Need a footnote (e.g. Github url) or a reference for Cubature.}
...
Spot size of $\rho = 10^{\circ}$ is considered (now defined in the spherical co-rotating coordinate system using linear length scales \red{TODO: What does this mean? Taken at face value, it seems to imply that we have non-linear length scales somewhere, or that the angular measurement $\rho$ is actually now defined with spatial, not angular units. Need to clarify.}), with varying co-latitudes ranging from near the pole at $\theta_{\mathrm{s}} = 10^{\circ}$ to the equator at $\theta_{\mathrm{s}} = 90^{\circ}$.
