...
We give detailed descriptions of the numerical algorithms used and provide an open source implementation of the numerical framework. %, available at \url{http://github.com/natj/bender}.
...
We have general relativistic effects such as bending of photon trajectories similar to black hole calculations, but, in addition, special relativistic effects like Doppler boosting and aberration of the observed angles, too.
...
Previous studies of emission from NSs are mainly formulated in a way that uses a curved spacetime metric for bending of the photon paths, with special relativistic corrections to radiative processes added separately, in an ad-hoc manner (see e.g., \citealt{PFC83, P95, WM01, PG03, PB06}, but also \citealt{BR01} for a general relativistic treatment).  %and propagate photons starting from the star itself.
...
We also assume metric signature of $(-,+,+,+)$ following the \citealt{MTW73} sign convention.
...
The total redshift as measured by at infinity is then given by an inner-product between a photon $u^{\alpha}$ and a 4-velocity of the star's fluid $s^{\alpha}$.
...
One should, however, note that by fixing the EoS, mass, and angular momentum, radius of the star is already determined if we consider realistic parameters for the star.
...
Hence, our $j$ and $q$ are smaller because we demand that radius should be $10\mathrm{km}$.
...
Hence, we are only able to reproduce the narrow peak shown in \citealt{BPO13} with enhanced quadrupole moment but not with a quadrupole moment value originating from our $q$ parameterization.

On the other hand, if we follow the parameter choices of \citealt{BPO13} and adopt $M = 1.4~\Msun$, $j = 0.357$ and $\nu = 700~\mathrm{Hz}$, our radius is fixed to be $R_{\mathrm{e}} \approx 11.8~\mathrm{km}$.
...
Hence, we conclude that for realistic NS parameters, one should never sees a narrow peak in the line profile even though some arbitrary parameter combinations of $M$, $R$ and $q$ do yield one.
